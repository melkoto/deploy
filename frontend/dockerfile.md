...
# Выполняем команду, которая выводит список файлов и директорий в `/app` после сборки, чтобы убедиться, что все собрано правильно.
RUN echo "Содержимое директории после сборки:" && ls -la /app
